...
const API_ENDPOINT = '/api/answer'; // ждём JSON { answer: "..." }
...
dialogSelectEl?.addEventListener('change', () => { state.activeId = dialogSelectEl.value; saveState(); renderAllMessages(); });
newDialogBtn?.addEventListener('click', () => {
  const id = uid();
  state.conversations.push({ id, title: 'Новый диалог', createdAt: nowISO(), messages: [] });
  state.activeId = id; saveState(); rebuildDialogSelect(); renderAllMessages();
});
...
clearDialogBtn?.addEventListener('click', () => {
  const conv = activeConversation(); if (!conv) return;
  if (confirm('Удалить все сообщения в текущем диалоге?')) { conv.messages = []; saveState(); renderAllMessages(); }
});
...
exportBtn?.addEventListener('click', () => {
  ...
  const a = document.createElement('a'); a.href = url; a.download = (conv.title || 'dialog') + '.json';
...
importBtn?.addEventListener('click', () => {
  ...
  input.addEventListener('change', async () => {
    ...
    try {
      const text = await file.text(); const conv = JSON.parse(text);
      if (!conv?.id || !Array.isArray(conv?.messages)) throw new Error('Неверный формат');
      state.conversations.push(conv); state.activeId = conv.id; saveState(); rebuildDialogSelect(); renderAllMessages();
...
inputEl?.addEventListener('input', () => {
  autoGrowTextarea(); charCounterEl.textContent = `${inputEl.value.length} / ${MAX_CHARS}`;
});
autoGrowTextarea(); charCounterEl.textContent = `${inputEl.value.length} / ${MAX_CHARS}`;

// Безопасный рендер Markdown + KaTeX
...
function enhanceContent(containerEl) {
  if (window.hljs) containerEl.querySelectorAll('pre code').forEach(block => { try { hljs.highlightElement(block); } catch {} });
...
function addMessageToState(role, content, extra = {}) {
  ...
  conv.messages.push(msg); saveState(); return msg;
}
function createMessageBubble(msg) {
  const div = document.createElement('div');
  div.className = `bubble ${msg.role === 'user' ? 'me' : 'bot'}`; div.dataset.id = msg.id; div.setAttribute('role', 'article');

  const actions = document.createElement('div'); actions.className = 'actions';
  const copyBtn = document.createElement('button'); copyBtn.className = 'icon-btn'; copyBtn.title = 'Копировать'; copyBtn.textContent = '📋';
  copyBtn.addEventListener('click', (e) => { e.stopPropagation(); navigator.clipboard.writeText(msg.content || ''); });
  actions.appendChild(copyBtn);

  // Регeнерация ответа (повторить последний вопрос)
  if (msg.role === 'bot') {
    const regenBtn = document.createElement('button');
    regenBtn.className = 'icon-btn'; regenBtn.title = 'Сгенерировать заново'; regenBtn.textContent = '🔁';
...
function addMessageToDOM(msg) { messagesEl.appendChild(createMessageBubble(msg)); messagesEl.scrollTop = messagesEl.scrollHeight; }
...
function toggleProgress(on) { if (!progressBar) return; progressBar.hidden = !on; progressBar.setAttribute('aria-hidden', String(!on)); }

// --- Запрос к бэкенду (JSON) ---
async function askBackendJSON(payload) {
  ...
  return res.json(); // ожидаем { answer: "..." }
}
...
formEl?.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const result = await askBackendJSON({
      question: text,
    ...
    const botText = (result && result.answer) ? result.answer : 'Ответ получен, но пустой.';
    const botMsg = addMessageToState('bot', botText);
    ...
    hideTypingIndicator(); toggleProgress(false);
  }
});
